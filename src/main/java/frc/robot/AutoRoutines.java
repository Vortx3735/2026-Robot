...
public class AutoRoutines {
    ...
    public AutoRoutine exampleRoutine() { 
        // Creates a routine called "example" and loads a trajectory. The trajectory is essentially the path the robot will take during auton. Look at https://choreo.autos/usage/editing-paths/#generating for an example.
        ...
        routine.active().onTrue(
            // Since onTrue only has one parameter, you need to use Commands.sequence to schedule more than one command.
            Commands.sequence(
                // No semicolons! Since you are passing arguments, it is only one statement. A semicolon only comes at the end of a statement. Also, since you are passing multiple arguments, you need commas.
